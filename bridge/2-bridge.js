...
async function pollWorker() {
  ...
  // v50 mutual single-poller lease: if cron is alive the worker's lastPoll is fresh (<150s) - stand down so the
  // feed pays ONE poll engine, not two (dual-source was the 2026-09-10 judge-cap burn multiplier). When cron
  // goes dark the freshness expires and this poller resumes as the fallback engine.
  try {
    const h = await fetch(WORKER_URL + '/health', { signal: AbortSignal.timeout(15000) });
    if (h.ok) {
      const hj = await h.json().catch(() => null);
      const lp = hj ? String(hj.lastPoll || '').split(' ')[0] : '';
      const ts = lp ? Date.parse(lp) : 0;
      if (ts && Date.now() - ts < 150000) return;
    }
  } catch (e) { /* worker unreachable - fall through and try the poll anyway */ }
  pollInFlight = true;
  try {
